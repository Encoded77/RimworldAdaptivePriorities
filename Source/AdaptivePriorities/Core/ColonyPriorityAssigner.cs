...
namespace AdaptivePriorities.Core
{
    ...
    public static class ColonyPriorityAssigner
    {
        ...
        /// <summary>Proposed priority (0 = unassigned) for every colonist and work type on the map.</summary>
        public static Dictionary<Pawn, Dictionary<WorkTypeDef, int>> ComputeProposal(Map map)
        {
            ...
            var capable = new List<(Pawn pawn, float score, bool opposed, bool inspired)>();
            ...
            foreach (var workType in workTypes)
            {
                ...
                foreach (var pawn in proposal.Keys)
                {
                    ...
                    float raw = PawnPriorityScorer.Score(pawn, workType);
                    colonyBest = Mathf.Max(colonyBest, raw);
                    capable.Add((pawn, raw, PawnPriorityScorer.IdeoOpposes(pawn, workType), PawnPriorityScorer.IsInspiredFor(pawn, workType)));
                }
                ...
                for (int i = 0; i < capable.Count; i++)
                    capable[i] = (capable[i].pawn, PawnPriorityScorer.BlendWithColonyBest(capable[i].score, colonyBest), capable[i].opposed, capable[i].inspired);
                ...
                {
                    var (pawn, score, opposed, inspired) = capable[rank];
                    ...
                    bool assigned = rank < maxWorkers
                                    && (policy.assignEveryone || score >= policy.scoreCutoff || rank < policy.minWorkers)
                                    && (!opposed || opposedCoverage);
                    ...
                    if (PriorityLockManager.IsCellLocked(pawn, workType))
